...
export class ApprovalResolver {
  constructor(@InjectManager() private readonly entityManager: EntityManager) {}
  ...
  async addApproval(@Arg('approval') approvalInput: ApprovalInput, @Ctx() { user }: Context): Promise<Approval> {
  ...
  @FieldResolver()
  isCanApprove(@Root() approval: Approval, @Ctx() { user }: Context) {
    return this.entityManager
      .findAndCount(Approver, {
        cache: 1000,
